...
const columns = 'id, album, artist, note, name, spotify_id AS spotifyId, apple_music_id AS appleMusicId, created_at AS createdAt';
const pageSize = 12;
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      try {
        ...
        const appleMusicId = field(body, 'appleMusicId', 16);
        if (appleMusicId && (!/^[1-9]\d{0,15}$/.test(appleMusicId) || !Number.isSafeInteger(Number(appleMusicId)))) throw new Error('Please choose the album again, or enter it manually.');
        record = { album: field(body, 'album', 120, true), artist: field(body, 'artist', 120, true), note: field(body, 'note', 500), name: field(body, 'name', 60), spotifyId: spotifyId || null, appleMusicId: appleMusicId || null };
      } catch (error) {
      ...
      const item = await db.prepare(`
        INSERT INTO recommendations (album, artist, note, name, spotify_id, apple_music_id, created_at, visitor_hash)
        SELECT ?, ?, ?, ?, ?, ?, ?, ? WHERE NOT EXISTS (
          SELECT 1 FROM recommendations WHERE visitor_hash = ? AND created_at > ?
        ) RETURNING ${columns}
      `).bind(record.album, record.artist, record.note, record.name, record.spotifyId, record.appleMusicId, now, hash, hash, now - 60000).first();
